(function () {
    ...
    function networkName(method) {
        if (!method) return '--';
        if (method.payment_kind === 'exchange') return method.token_net_name || (method.provider === 'okx' ? 'OKX Pay' : 'Binance Pay');
    ...
    function safeHttpsUrl(url, fallback) {
    ...
    function hydrateOrder(order) {
        orderData = order || {};
        config.return_url = safeHttpsUrl(orderData.redirect_url, '');
        ...
        configureManualTxSubmit(orderData);
        startCountdown();
        updateReselectControls();
    }

    function configureManualTxSubmit(data) {
        var section = $('#manualTxSection');
        var form = $('#manualTxForm');
        var toggle = $('#manualTxToggle');
        var input = $('#manualTxInput');
        var submit = $('#manualTxSubmit');
        var message = $('#manualTxMessage');
        if (!section || !form || !toggle || !input || !submit || !message) return;
        ...
        form.addEventListener('submit', function (event) {
            ...
            if (!txHash) {
                message.textContent = '请输入交易哈希';
                message.classList.add('error');
                return;
            }

            submit.disabled = true;
            message.textContent = '正在验证链上交易...';
            apiPost('/api/v1/pay/submit-tx-hash', {
                trade_id: (orderData && orderData.trade_id) || tradeId,
                tx_hash: txHash
            })
                .then(function () {
                    input.disabled = true;
                    message.textContent = '已提交，正在确认链上交易';
                ...
                .catch(function (error) {
                    ...
                    message.textContent = error.message || '交易验证失败';
                    message.classList.add('error');
                });
        });
    }
    ...
    function renderPaymentDetail() {
        ...
        var addressLabel = document.querySelector('.address-box label');
        var copyAddressButton = $('#copyAddressButton');
        var transferTitle = document.querySelector('.transfer-console .pay-head h2');
        var isExchange = paymentDetail.payment_kind === 'exchange' || paymentDetail.provider === 'binance' || paymentDetail.provider === 'okx';
        var providerName = paymentDetail.provider === 'okx' ? 'OKX Pay' : 'Binance Pay';
        ...
        if (addressLabel) addressLabel.textContent = isExchange ? (paymentDetail.receiver_label || providerName + ' UID') : t('transfer.address');
        if (copyAddressButton) copyAddressButton.textContent = isExchange ? '复制 UID' : t('actions.copyAddress');
        if (transferTitle) transferTitle.textContent = isExchange ? '在 ' + providerName + ' 内完成转账' : t('transfer.title');
        if (networkInstruction) networkInstruction.textContent = isExchange ? '使用 ' + providerName + ' 内部转账到指定 UID' : t('instruction.networkUse', {network: networkText});
    ...
    function startCountdown() {
        ...
        countdownTimer = setInterval(function () {
            ...
            if (totalSeconds <= 0) {
                ...
                if (!orderData || !orderData.manual_tx_submit) {
    ...
    function showTimeout() {
        ...
        showOverlay('timeout', t('overlay.timeoutTitle'), t('overlay.timeoutBody'), t('overlay.returnMerchant'), function () {
            window.location.href = safeHttpsUrl(config.return_url, '/');
        });
    }
    ...
    function showCanceled(data) {
        ...
        showOverlay('timeout', t('overlay.canceledTitle'), t('overlay.canceledBody'), t('overlay.returnMerchant'), function () {
            window.location.href = safeHttpsUrl((data && data.redirect_url) || config.return_url, '/');
        });
    }
    ...
    function showSuccess(data) {
        ...
        showOverlay('success', t('overlay.successTitle'), t('overlay.successBody'), t('overlay.returnMerchant'), function () {
            window.location.href = safeHttpsUrl(data.redirect_url || data.return_url || config.return_url, '/');
        });
    }
    ...
    function loadOrder() {
        return apiPost('/api/v1/pay/info', {trade_id: tradeId})
            .then(function (data) {
                ...
                if (data.status === 3 && !data.manual_tx_submit) {
                    showTimeout();
                    return;
                }
                if (data.status === 3) setStatusKey('status.timeout');
                if (data.status === 4) {
                    showCanceled(data);
                    return;
                }
