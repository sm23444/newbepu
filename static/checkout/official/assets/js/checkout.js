(function () {
    ...
    function showTimeout() {
        ...
        var timeoutReturn = ov.querySelector('.return-btn');
...
(function () {
    ...
    function renderReceiver(d) {
        var isExchange = d.payment_kind === 'exchange' || d.provider === 'binance' || d.provider === 'okx';
        var stage = document.getElementById('paymentStage');
        var qrWrapper = stage ? stage.querySelector('.qr-wrapper') : null;
        var addressCard = stage ? stage.querySelector('.address-card') : null;
        var notice = document.getElementById('exchangePaymentNotice');
        var addr = document.getElementById('addressLabelQ');
        var receiver = d.token || d.address || '';

        if (!isExchange) {
            if (qrWrapper) qrWrapper.style.display = '';
            if (notice) notice.remove();
            if (addr) addr.textContent = _t('receivingAddress', '收款地址');
            $('#qrcode').empty().qrcode({ text: receiver, width: 200, height: 200 });
            return;
        }

        if (qrWrapper) qrWrapper.style.display = '';
        if (!notice && addressCard) {
            notice = document.createElement('div');
            notice.id = 'exchangePaymentNotice';
            notice.style.cssText = 'margin:0 0 14px;padding:14px 16px;border:1px solid #e5e7eb;border-radius:6px;background:#f8fafc;color:#334155;line-height:1.65;text-align:left;';
            addressCard.parentNode.insertBefore(notice, addressCard);
        }
        if (notice) {
            var provider = d.provider === 'okx' ? 'OKX Pay' : 'Binance Pay';
            notice.innerHTML = '<strong style="display:block;color:#111827;margin-bottom:4px;">' + provider + ' 内部转账</strong>' +
                '<span>请在 ' + provider + ' 中向下方 UID 转入页面显示的准确金额，币种选择 USDT。</span>';
        }
        if (addr) addr.textContent = d.receiver_label || (d.provider === 'okx' ? 'OKX UID' : 'Binance UID');
        $('#qrcode').empty().qrcode({ text: receiver, width: 200, height: 200 });
    }
