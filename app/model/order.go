...
type Order struct {
	...
	NotifyClaimToken  string     `gorm:"column:notify_claim_token;type:varchar(64);not null;default:'';comment:回调领取令牌" json:"-"`
	NotifyClaimUntil  *time.Time `gorm:"column:notify_claim_until;index:idx_order_notify_claim_until;comment:回调领取截止时间" json:"-"`
...
func (o *Order) CanReselectPayment() bool {
	...
	if IsExchangeTradeType(o.TradeType) {
		return false
	}
...
func (o *Order) SetSuccess() error {
	result := Db.Model(&Order{}).
		Where("id = ? AND status = ? AND ref_hash = ?", o.ID, OrderStatusConfirming, o.RefHash).
		Update("status", OrderStatusSuccess)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected == 0 {
		return fmt.Errorf("order %d is not confirming transaction %s", o.ID, o.RefHash)
	}
...
func (o *Order) ClaimNotify(force bool) (Order, error) {
	claimToken, err := utils.GenerateTradeId()
	if err != nil {
		return Order{}, fmt.Errorf("generate notify claim token: %w", err)
	}

	now := time.Now()
	claimUntil := now.Add(notifyClaimDuration)
	query := Db.Model(&Order{}).
		Where("id = ? AND status = ?", o.ID, OrderStatusSuccess).
		Where("notify_claim_until IS NULL OR notify_claim_until < ?", now)
	if !force {
		...
		query = query.
			...
			Where("notify_num <= ?", maxRetry)
	}

	result := query.Updates(map[string]any{
		"notify_claim_token": claimToken,
	...
	if result.RowsAffected != 1 {
		return Order{}, fmt.Errorf("order %d notification is already claimed or not eligible", o.ID)
	}

	var claimed Order
	if err := Db.First(&claimed, o.ID).Error; err != nil {
		return Order{}, err
	}
	*o = claimed
	return claimed, nil
...
func (o *Order) CompleteNotify(success bool) error {
	updates := map[string]any{
		"notify_claim_token": "",
		"notify_claim_until": nil,
	}
	if success {
		updates["notify_state"] = OrderNotifyStateSucc
	}

	result := Db.Model(&Order{}).
		Where("id = ? AND notify_claim_token = ?", o.ID, o.NotifyClaimToken).
		Updates(updates)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected != 1 {
		return fmt.Errorf("order %d notification claim is no longer owned", o.ID)
	}

	o.NotifyClaimToken = ""
	o.NotifyClaimUntil = nil
	if success {
		o.NotifyState = OrderNotifyStateSucc
	}
	return nil
}
...
func (o *Order) RedirectUrl() string {
	if !utils.IsAllowedHTTPSURL(o.ReturnUrl) {
		return ""
	}
	...
	if o.Status == OrderStatusSuccess && o.ApiType == OrderApiTypeEpay {
		var err error
		redirect, err = utils.AppendURLQuery(redirect, o.BuildNotifyParams())
		if err != nil {
			return ""
		}
	}
...
func (o *Order) GetMethods(crypto Crypto) []MethodItem {
	...
	for tradeTypeStr, typeConf := range allTrades {
		if IsExchangeTradeType(TradeType(tradeTypeStr)) && !IsExchangeProviderEnabled(TradeType(tradeTypeStr)) {
			continue
		}
		// 如果指定了货币，则进行过滤
		...
		items = append(items, MethodItem{
			...
			TokenCustomName: typeConf.NetworkName,
			IsPopular:       false,
			PaymentKind:     paymentKind(TradeType(tradeTypeStr)),
			Provider:        exchangeProvider(TradeType(tradeTypeStr)),
			ReceiverLabel:   receiverLabel(TradeType(tradeTypeStr)),
		})
	}
...
func (o *Order) Network() any {
	...
	net.PaymentKind = paymentKind(o.TradeType)
	net.Provider = exchangeProvider(o.TradeType)
	net.ReceiverLabel = receiverLabel(o.TradeType)
...
func GetNotifyFailedTradeOrders() ([]Order, error) {
	...
	res := Db.Where("status = ?", OrderStatusSuccess).
		Where("notify_state = ?", OrderNotifyStateFail).
		Where("notify_num <= ?", maxRetry).
		Where("notify_claim_until IS NULL OR notify_claim_until < ?", time.Now()).
		Find(&orders)
...
func (o *Order) PaymentProvider() string {
	return exchangeProvider(o.TradeType)
}

func (o *Order) ReceiverLabel() string {
	return receiverLabel(o.TradeType)
}
