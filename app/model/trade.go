...
func StartBuildOrder(p OrderParams) (Order, error) {
	...
	if p.Address != "" {
		if IsExchangeTradeType(p.TradeType) {
			if _, err := NewWallet(p.Address, p.TradeType); err != nil {
				return order, err
			}
		} else if !utils.IsValidTronAddress(p.Address) &&
...
func BuildTrade(p OrderParams) (Trade, error) {
	if IsExchangeTradeType(p.TradeType) && !IsExchangeProviderEnabled(p.TradeType) {
		return Trade{}, fmt.Errorf("%s payment provider is disabled", p.TradeType)
	}
...
func RebuildOrder(t Order, p OrderParams) (Order, error) {
	...
	var current Order
	res := Db.Where("id = ?", t.ID).Limit(1).Find(&current)
	if res.Error != nil {
		return t, res.Error
	}
	if res.RowsAffected == 0 {
		return t, fmt.Errorf("order not found")
	}

	return rebuildOrder(current, p)
}

// rebuildOrder must be called while buildMutex is held so amount allocation
// and the matching order update remain serialized.
func rebuildOrder(t Order, p OrderParams) (Order, error) {
	if p.OrderId == t.OrderId && p.TradeType == t.TradeType && p.Money.String() == t.Money && p.Fiat == t.Fiat {
		return t, nil
	}
	if t.Status != OrderStatusWaiting {
		return t, fmt.Errorf("order status does not allow payment updates")
	}
	if IsExchangeTradeType(t.TradeType) {
	...
	query := Db.Model(&Order{}).
		...
		Where("trade_type = ?", current.TradeType).
		Where("fiat = ?", current.Fiat).
		Where("crypto = ?", current.Crypto).
		Where("rate = ?", current.Rate).
	...
	if res.RowsAffected == 0 {
		return current, fmt.Errorf("order payment state changed or client fingerprint mismatch")
	}
