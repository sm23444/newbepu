...
func (Order) Create(ctx *gin.Context) {
	...
	if err := updateOrderURLs(&order); err != nil {
		base.Response(ctx, 400, gin.H{
			"status":  "failed",
...
func updateOrderURLs(order *model.Order) error {
...
func (Order) Paid(ctx *gin.Context) {
	...
	updated, err := markOrderPaid(&order, req.RefHash, confirmedAt)
	if err != nil {
		base.Error(ctx, err)
	...
	if !updated {
		base.BadRequest(ctx, "订单状态已变化或不允许补单，请刷新后重试")
...
func markOrderPaid(order *model.Order, refHash string, confirmedAt time.Time) (bool, error) {
	if order.Status == model.OrderStatusSuccess || order.Status == model.OrderStatusCanceled {
		return false, nil
	}
	...
	result := model.Db.Model(&model.Order{}).
		Where("id = ? AND status = ? AND ref_hash = ?", order.ID, order.Status, order.RefHash).
		Updates(update)
	if result.Error != nil {
...
func (Order) ManualNotify(ctx *gin.Context) {
	...
	if err := notify.HandleManual(order); err != nil {
		base.BadRequest(ctx, err.Error())
