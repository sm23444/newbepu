...
func (t *tron) parseContractTransfers(contract *core.Transaction_Contract, id string, num int, timestamp time.Time) []transfer {
	...
	if contract.GetType() == core.Transaction_Contract_TransferContract {
		...
		if err := contract.GetParameter().UnmarshalTo(foo); err != nil || foo.Amount <= 0 {
			return transfers
		}
	...
	switch {
	case bytes.Equal(foo.GetContractAddress(), usdtTrc20ContractAddress):
		tradeType = model.UsdtTrc20
	case bytes.Equal(foo.GetContractAddress(), usdcTrc20ContractAddress):
		tradeType = model.UsdcTrc20
	default:
		return transfers
	}
	...
	switch {
	case bytes.Equal(data[:4], []byte{0xa9, 0x05, 0x9c, 0xbb}):
		receiver, amount := t.parseTrc20ContractTransfer(data)
		if amount != nil && amount.Sign() > 0 {
			transfers = append(transfers, transfer{
				Network:     conf.Tron,
	...
	case bytes.Equal(data[:4], []byte{0x23, 0xb8, 0x72, 0xdd}):
...
func (t *tron) tradeConfirmHandle(ctx context.Context) {
	...
	var handle = func(o model.Order) {
		if model.GetC(model.BlockOffsetConfirm) == "1" {
			...
			lastBlockNum := t.lastBlockNum
			t.heightMu.RUnlock()
			if lastBlockNum == 0 || lastBlockNum-o.RefBlockNum < t.blockConfirmedOffset {
				return
			}
		}
		...
		if o.TradeType == model.TronTrx {
			...
			if tronTransactionSucceeded(trans) {
...
func tronTransactionSucceeded(trans *core.Transaction) bool {
	if trans == nil {
		return false
	}
	ret := trans.GetRet()
