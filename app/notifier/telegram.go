...
func (t *Telegram) NotifyFail(o model.Order, reason string) {
	tradeType := string(o.TradeType)
	text, err := notifyFailText(o, reason)
	if err != nil {
		t.sendMessage(&bot.SendMessageParams{Text: "❌交易类型不支持：" + tradeType})
	...
	t.sendMessage(orderSendMessageParams(text, o, "📝查看收款详情"))
}

func orderSendMessageParams(text string, o model.Order, label string) *bot.SendMessageParams {
	params := &bot.SendMessageParams{
		Text:      text,
		ParseMode: models.ParseModeMarkdown,
	}
	if markup := orderTransactionReplyMarkup(o, label); markup != nil {
...
func notifyFailText(o model.Order, reason string) (string, error) {
	tradeType := string(o.TradeType)
...
func (t *Telegram) Welcome() {
	text := `
👋 欢迎使用 smpay，` + conf.Desc + `，如果您看到此消息，说明系统已启动成功！
